...
class CartController extends Controller
{
    ...
    public function add_to_cart(Product $product, Request $request)
    {
        ...
        if ($existing_product == null) {
            ...
            $request->validate([
                'amount' => 'required|gte:1|lte:' . ($product->stock - $existing_product->amount)
            ]);

            $existing_product->update([
                'amount' => $existing_product->amount + $request->amount
            ]);
        }
    ...
    public function edit_cart(Cart $cart, Request $request)
    {
        $request->validate([
